 ...
 *
 * The standard TesterEnvironment<> class can't handle gar::geo::GeometryCore.
...
namespace testing {
   ...
   * that require the presence of geometry (in the form of gar::geo::GeometryCore
   * instance).
   *
    ...
    virtual public TesterEnvironment<ConfigurationClass>
  {
    ...
    gar::geo::GeometryCore const* Geometry() const { return geom.get(); }
   ...
   */
  template <typename ConfigurationClass>
  std::unique_ptr<gar::geo::GeometryCore>
  GeometryTesterEnvironment<ConfigurationClass>::CreateNewGeometry() const
  {
    ...
    auto new_geom = std::make_unique<gar::geo::GeometryCore>(ProviderConfig);
    ...
    std::shared_ptr<gar::geo::seg::ChannelMapAlg> pChannelMap
  ...
  void GeometryTesterEnvironment<ConfigurationClass>::SetupGeometry() {
    ...
    // new
    this->template AcquireProvider<gar::geo::GeometryCore>(CreateNewGeometry());
