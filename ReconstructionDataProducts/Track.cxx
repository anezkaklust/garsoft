...
namespace gar {
    namespace rec {
        ...
        : fLengthforwards(trk.fLengthforwards),
        fLengthbackwards(trk.fLengthbackwards),
        fMomentum_beg(trk.fMomentum_beg),
        fMomentum_end(trk.fMomentum_end),
        fChisqForward(trk.fChisqForward),
        fChisqBackward(trk.fChisqBackward),
        fNHits(trk.fNHits),
        fTime(trk.fTime)
        {
            for(unsigned int i = 0; i < 3; i++)
            {
                fVertex[i] = trk.fVertex[i];
                fEnd[i] = trk.fEnd[i];
                fVtxDir[i] = trk.fVtxDir[i];
                fEndDir[i] = trk.fEndDir[i];
            }

            for(unsigned int i = 0; i < 5; i++)
            {
                fTrackParBeg[i] = trk.fTrackParBeg[i];
                fTrackParEnd[i] = trk.fTrackParEnd[i];
            }
            for(unsigned int i = 0; i < 15; i++)
            {
                fCovMatBeg[i] = trk.fCovMatBeg[i];
                fCovMatEnd[i] = trk.fCovMatEnd[i];
            }
        }

        //--------------------------------------------------------------------------
        void Track::operator= (const Track& trk)
        {
            fLengthforwards = trk.fLengthforwards;
            fLengthbackwards = trk.fLengthbackwards;
            fMomentum_beg = trk.fMomentum_beg;
