...
//
//    THIS VERSION OBSOLETE AFTER INTRODUCTION OF IDNumbers	IN 
//    RECONSTRUCTEDDATAPRODUCTS CLASSES esp. TRACK, VERTEX, CLUSTER
//    IT IS RETAINED IN THE REPOSITORY FOR BACKWARDS COMPATIBILITY
//
////////////////////////////////////////////////////////////////////////
...
namespace gar {
    ...
    private:
        ...
        std::vector<Float_t>     fVTAssn_TrkEndQ;
...
void gar::anatree::beginJob() {
    ...
    if (fWriteVertsNtracks) {                     // Reco'd verts & their track-ends
        ...
        fTree->Branch("VT_TrkEndQ",      &fVTAssn_TrkEndQ);
        ...
        for (size_t imchl = 0; imchl < mcthandlelist.size(); ++imchl)
        {
            for ( auto const& mct : (*mcthandlelist.at(imchl)) ) {
                if (mct.NeutrinoSet())
                {
                    ...
                    if (fWriteCohInfo) {
                        double getT = gar::computeT_shrub(mct);
    ...
    if (fWriteAllTracks) {
        ...
        for ( auto const& track : (*TrackHandle) ) {
            ...
            if (findIonization.isValid()) {
                ...
                gar::processIonizationInfo_shrub(ionization, fIonizTruncate, avgIonF, avgIonB);
