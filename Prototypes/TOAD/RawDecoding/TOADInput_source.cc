	  ...
	  if(subdetector_string == "ND_GAr")
            {
              ...
              MF_LOG_INFO("HDF5") << "frag_size"<<(int)frag_size<<(int)fhs;
              int frag_start = 0;
              int frag_interval=0;
              gar::raw::ADCvector_t adc_vector;
              MF_LOG_INFO("HDF5") << "Here 12";

              for(frag_start = 0; frag_start<=(int)(frag_size+fhs); frag_start += frag_interval)
                {
                ...
                gar::raw::Compress_t compress = gar::raw::kNone;
                ULong64_t time = 0;
