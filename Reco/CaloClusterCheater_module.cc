...
namespace gar {
    namespace rec {
        ...
        class eveLoc {
        public:
            eveLoc(int id, gar::raw::CellID_t cID)
            : eveID(id), cellID(cID)
            {}
            ...
            gar::raw::CellID_t GetCellID() const { return cellID; }

        private:
            int                    eveID;
            gar::raw::CellID_t     cellID;
        };
        ...
        private:
            ...
            gar::rec::Cluster CreateCluster(std::vector< art::Ptr<gar::rec::CaloHit> > hitVec);
        };
        ...
        EDProducer{pset}
        {
            fMCGeneratorLabel      = pset.get< std::string  >("MCGeneratorLabel",   "genie"    );
        ...
        void CaloClusterCheater::produce(art::Event & e)
        {
            ...
            this->CollectHits(e, fCaloHitModuleLabel, artHits);
            ...
            for( auto const& itr : artHits ) {
                ...
                for(size_t ieve = 0; ieve < eveides.size(); ieve++) {
                    ...
                    eveLoc el(eveides[ieve].trackID, itr->CellID());
            ...
            for(auto hitMapItr : eveCaloHitMap) {
                ...
                gar::rec::Cluster cluster = this->CreateCluster(hitMapItr.second);
                ...
                LOG_INFO("CaloClusterCheater") << "adding cluster: \n" << clustercol->back() << "\nto collection.";
        ...
        //-----------------------------------------------------------------------------------
        void CaloClusterCheater::CollectHits(const art::Event &evt, const std::string &label, std::vector< art::Ptr<gar::rec::CaloHit> > &hitVector)
        {
        ...
        //-----------------------------------------------------------------------------------
        gar::rec::Cluster CaloClusterCheater::CreateCluster(std::vector< art::Ptr<gar::rec::CaloHit> > hitVec)
        {
            gar::rec::Cluster clu;
