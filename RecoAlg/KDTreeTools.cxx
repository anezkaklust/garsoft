...
namespace gar {
    namespace rec {
        namespace alg {
            ...
            std::pair<double, double> minmax(const double a, const double b)
            {
                return ((b < a) ? std::pair<double, double>(b, a) : std::pair<double, double>(a, b));
            }


            //------------------------------------------------------------------------------------------------------------------------------------------

            KDTreeTesseract fill_and_bound_4d_kd_tree(const std::list<const gar::rec::CaloHit *> &points, std::vector<KDTreeNodeInfoT<const gar::rec::CaloHit*, 4> > &nodes)
            {
                std::array<double, 4> minpos{ {0.f, 0.f, 0.f, 0.f} }, maxpos{ {0.f, 0.f, 0.f, 0.f} };

                unsigned i = 0;

                for (const gar::rec::CaloHit *const point : points)
                {
                    const CLHEP::Hep3Vector &pos = kdtree_type_adaptor<const gar::rec::CaloHit>::position(point);
            ...
            KDTreeCube build_3d_kd_search_region(const gar::rec::CaloHit *const point, const float x_span, const float y_span, const float z_span)
            {
            ...
            KDTreeTesseract build_4d_kd_search_region(const gar::rec::CaloHit *const point, const float x_span, const float y_span, const float z_span, const float search_layer)
            {
