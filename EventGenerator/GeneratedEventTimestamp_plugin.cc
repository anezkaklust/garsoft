...
namespace gar{
  ...
  namespace evgen {
    namespace details {
      ...
      template <typename Clock, typename Duration>
      class TimeInUnits<Clock, Duration, typename std::enable_if<(Clock::period::num * Duration::period::den > Duration::period::num * Clock::period::den)>::type>
      : public TimeInUnitsBase<Clock, Duration>
      {
        ...
        static constexpr intmax_t paddingfactor = PeriodRatio::num / PeriodRatio::den; // > 1 enforced by enable_if<>
